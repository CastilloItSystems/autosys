...
import {
  findAllQualityChecks, findQualityCheckById, findQualityCheckBySOId,
...
export const getAll = async (req: Request, res: Response) => {
  const result = await findAllQualityChecks(prisma, req.empresaId!, req.validatedQuery as any)
...
export const create = async (req: Request, res: Response) => {
  ...
  return ApiResponse.created(res, new QualityCheckResponseDTO(item), WORKSHOP_MESSAGES.qualityCheck.created)
}
...
export const submit = async (req: Request, res: Response) => {
  ...
  const msg = item.status === 'PASSED' ? WORKSHOP_MESSAGES.qualityCheck.passed : WORKSHOP_MESSAGES.qualityCheck.failed
