...
export const getByServiceOrder = async (req: Request, res: Response) => {
  const item = await findQualityCheckBySOId(prisma, req.params.serviceOrderId, req.empresaId!)
  return ApiResponse.success(res, new QualityCheckResponseDTO(item))
}

export const getOne = async (req: Request, res: Response) => {
  const item = await findQualityCheckById(prisma, req.params.id, req.empresaId!)
  return ApiResponse.success(res, new QualityCheckResponseDTO(item))
}
...
export const submit = async (req: Request, res: Response) => {
  const item = await submitQualityCheck(prisma, req.params.id, req.empresaId!, new SubmitQualityCheckDTO(req.body))
