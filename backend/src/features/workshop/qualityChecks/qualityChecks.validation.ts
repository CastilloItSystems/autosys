...
const checklistItemSchema = Joi.object({
  ...
  notes: Joi.string().trim().max(500).optional().allow('').allow(null),
})

export const createQualityCheckSchema = Joi.object({
  serviceOrderId: Joi.string()
    .required()
    .messages({ 'any.required': 'La orden de trabajo es requerida' }),
  inspectorId: Joi.string()
    .required()
    .messages({ 'any.required': 'El inspector es requerido' }),
