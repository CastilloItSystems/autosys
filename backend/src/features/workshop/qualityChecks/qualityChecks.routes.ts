...
} from './qualityChecks.validation.js'
...
router.get(
  '/by-order/:serviceOrderId',
  authorize(PERMISSIONS.WORKSHOP_VIEW),
  asyncHandler(ctrl.getByServiceOrder)
)
// CRUD directo por ID
router.post(
  '/',
  authorize(PERMISSIONS.WORKSHOP_UPDATE),
  validateBody(createQualityCheckSchema),
...
router.patch(
  '/:id/submit',
  authorize(PERMISSIONS.WORKSHOP_UPDATE),
  validateBody(submitQualityCheckSchema),
