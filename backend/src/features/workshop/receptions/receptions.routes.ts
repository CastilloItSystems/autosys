...
import {
  ...
  changeReceptionStatusSchema,
} from './receptions.validation.js'
...
router.get(
  ...
  validateQuery(receptionFiltersSchema),
  asyncHandler(ctrl.getAll)
)
router.post(
  '/',
  authorize(PERMISSIONS.WORKSHOP_CREATE),
  validateBody(createReceptionSchema),
...
router.put(
  ...
  validateBody(updateReceptionSchema),
  asyncHandler(ctrl.update)
)
router.patch(
  '/:id/status',
  authorize(PERMISSIONS.WORKSHOP_UPDATE),
  validateBody(changeReceptionStatusSchema),
  asyncHandler(ctrl.changeStatus)
)
router.get(
  '/:id/checklist-responses',
  authorize(PERMISSIONS.WORKSHOP_VIEW),
  asyncHandler(ctrl.getChecklistResponses)
)
router.post(
  '/:id/checklist-responses',
  authorize(PERMISSIONS.WORKSHOP_UPDATE),
  asyncHandler(ctrl.saveChecklistResponses)
)
router.post(
  '/:id/signature',
  authorize(PERMISSIONS.WORKSHOP_UPDATE),
  asyncHandler(ctrl.uploadSignature)
)
router.delete(
  '/:id',
