...
export async function findAllTOTs(
  db: Db,
  empresaId: string,
  filters: ITOTFilters
) {
...
export async function createTOT(
  db: Db,
  empresaId: string,
  userId: string,
  data: ICreateTOT
  ...
  if (data.supplierId) {
    const sup = await (db as PrismaClient).supplier.findFirst({
...
export async function updateTOT(
  db: Db,
  id: string,
  empresaId: string,
  data: IUpdateTOT
  ...
  if (item.status === 'INVOICED' || item.status === 'CANCELLED') {
    throw new BadRequestError(
      'No se puede editar un T.O.T. en estado INVOICED o CANCELLED'
    )
  }
  if (data.supplierId) {
    const sup = await (db as PrismaClient).supplier.findFirst({
  ...
  // Calculate taxAmount if clientPrice changes
  const updateData: any = {
    ...
    ...(data.requestedWork !== undefined && {
      requestedWork: data.requestedWork,
    }),
    ...(data.technicalInstruction !== undefined && {
      technicalInstruction: data.technicalInstruction,
    }),
...
export async function updateTOTStatus(
  db: Db,
  id: string,
  empresaId: string,
  newStatus: TOTStatus
  ...
  if (newStatus === 'DEPARTED' && !item.departedAt)
    extraData.departedAt = new Date()
  if (newStatus === 'RETURNED' && !item.returnedAt)
  ...
  if (newStatus === 'INVOICED') {
    ...
    if (cost > 0) {
      const updated = await (db as PrismaClient).$transaction(async (tx) => {
        const tot = await tx.workshopTOT.update({
          where: { id },
        ...
        const prevCost = Number(
          otherTOTs._sum.finalCost ?? otherTOTs._sum.providerQuote ?? 0
        )
        ...
        if (so) {
          ...
          const newSubtotal =
            Number(so.laborTotal) + Number(so.partsTotal) + newOtherTotal
      ...
      // Trigger billing sync after update
      if (db instanceof PrismaClient) {
        try {
          await syncAfterTOTChange(db, id)
        } catch (err: any) {
          console.error(
            `[workshop-tot] Error syncing billing for TOT ${id}:`,
            err?.message
          )
        }
      }
    ...
    // Trigger billing sync when TOT returns (so clientPrice can be set separately)
    const updated = await (db as PrismaClient).workshopTOT.update({
      where: { id },
...
export async function removeDocument(
  ...
  const doc = await (db as PrismaClient).workshopTOTDocument.findFirst({
    where: { id: docId, totId },
  })
  if (!doc) throw new NotFoundError('Documento no encontrado')
  await (db as PrismaClient).workshopTOTDocument.delete({
    where: { id: docId },
  })
}
...
export async function removeTOT(db: Db, id: string, empresaId: string) {
  ...
  if (!['REQUESTED', 'CANCELLED'].includes(item.status)) {
    throw new ConflictError(
      'Solo se pueden eliminar T.O.T. en estado REQUESTED o CANCELLED'
    )
  }
