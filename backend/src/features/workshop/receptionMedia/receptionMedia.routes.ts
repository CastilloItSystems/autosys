...
import {
  ...
  updateDamageSchema,
  createPhotoSchema,
} from './receptionMedia.validation.js'
...
router.post(
  ...
  FileUploadHelper.createMemoryUploader('file'),
...
router.get(
  '/photos',
  authorize(PERMISSIONS.WORKSHOP_VIEW),
  asyncHandler(ctrl.getPhotos)
)
router.post(
  '/photos',
  authorize(PERMISSIONS.WORKSHOP_CREATE),
  validateBody(createPhotoSchema),
  asyncHandler(ctrl.addPhoto)
)
router.delete(
  '/photos/:photoId',
  authorize(PERMISSIONS.WORKSHOP_DELETE),
  asyncHandler(ctrl.removePhoto)
)
