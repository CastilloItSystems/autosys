...
export const createLoan = asyncHandler(async (req: Request, res: Response) => {
  ...
  const loan = await service.create(dto, req.user?.userId || 'SYSTEM')
...
export const updateLoan = asyncHandler(async (req: Request, res: Response) => {
  ...
  const loan = await service.update(id, dto, req.user?.userId || 'SYSTEM')
...
export const activateLoan = asyncHandler(
  async (req: Request, res: Response) => {
    const id = req.params.id as string
    const loan = await service.activate(id, req.user?.userId || 'SYSTEM')
...
export const returnLoanItems = asyncHandler(
  async (req: Request, res: Response) => {
    ...
    const loan = await service.returnItems(id, items, req.user?.userId || 'SYSTEM')
