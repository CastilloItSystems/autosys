...
import { ICreateLoanInput, IUpdateLoanInput, IReturnLoanInput, ILoanResponseDTO, LoanStatus } from './loans.interface';
...
export class LoanResponseDTO implements ILoanResponseDTO {
  ...
  warehouseId: string;
  status: LoanStatus;
  items: Array<{ itemId: string; quantityLoaned: number; quantityReturned: number }>;
  startDate: Date;
  dueDate: Date;
  returnedAt?: Date;
  daysOverdue?: number;
    ...
    if (data.status === 'OVERDUE' || data.status === 'ACTIVE') {
      ...
      if (now > new Date(data.dueDate)) {
        this.daysOverdue = Math.floor(
          (now.getTime() - new Date(data.dueDate).getTime()) / (1000 * 60 * 60 * 24)
        );
      }
    }
  }
