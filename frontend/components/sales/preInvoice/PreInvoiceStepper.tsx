...
import {
  ...
  PREINVOICE_STATUS_STEPS,
} from "@/libs/interfaces/sales/preInvoice.interface";

interface PreInvoiceStepperProps {
  currentStatus: PreInvoiceStatus;
}

const PreInvoiceStepper = ({ currentStatus }: PreInvoiceStepperProps) => {
  const isCancelled = currentStatus === PreInvoiceStatus.CANCELLED;
  const currentIdx = PREINVOICE_STATUS_STEPS.indexOf(currentStatus);
  ...
  return (
    <div className="flex align-items-center justify-content-center py-2 gap-0">
      {PREINVOICE_STATUS_STEPS.map((step, idx) => {
        const config = PREINVOICE_STATUS_CONFIG[step];
        ...
        return (
          <React.Fragment key={step}>
            ...
            {idx < PREINVOICE_STATUS_STEPS.length - 1 && (
              <div
                className={`flex-1 ${
...
export default PreInvoiceStepper;
