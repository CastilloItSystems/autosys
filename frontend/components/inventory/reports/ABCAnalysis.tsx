...
const ABCAnalysis = () => {
  ...
  const loadABCAnalysis = async () => {
    setLoading(true);
    try {
      const response = await analyticsService.getABCAnalysis({
        page,
        limit: rows,
      });
        ...
        <Card title="Análisis de Pareto - Clasificación ABC">
          ...
          <div className="mt-3 flex flex-column gap-2">
            <p className="text-sm text-500 m-0">
              <strong>Clase A:</strong> Los 20% de artículos que representan el
              80% del valor total. Requieren control estricto y pronóstico de
              demanda.
            </p>
            <p className="text-sm text-500 m-0">
              <strong>Clase B:</strong> Los artículos intermedios que
              representan el 15% del valor total. Control estándar.
            </p>
            <p className="text-sm text-500 m-0">
              <strong>Clase C:</strong> Los artículos que representan solo el 5%
              del valor total. Control simplificado.
            </p>
          </div>
        </Card>
      )}
