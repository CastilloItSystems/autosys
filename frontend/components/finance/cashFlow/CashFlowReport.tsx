...
import type { CashTransaction, CashFlowSummary, CashFlowCurrencySummary } from "@/libs/interfaces/finance";
...
function SummaryCards({ s, fmt }: { s: CashFlowCurrencySummary; fmt: (v: number) => string }) {
  return (
    <div className="grid">
      <div className="col-12 md:col-4">
        <Card className="border-left-3 border-green-400">
          <div className="text-color-secondary mb-1 text-sm">Total Entradas</div>
          <div className="text-2xl font-bold text-green-600">{s.currency} {fmt(s.totalIncome)}</div>
        </Card>
      </div>
      <div className="col-12 md:col-4">
        <Card className="border-left-3 border-red-400">
          <div className="text-color-secondary mb-1 text-sm">Total Salidas</div>
          <div className="text-2xl font-bold text-red-600">{s.currency} {fmt(s.totalOutcome)}</div>
        </Card>
      </div>
      <div className="col-12 md:col-4">
        <Card className={`border-left-3 ${s.netFlow >= 0 ? "border-primary" : "border-orange-400"}`}>
          <div className="text-color-secondary mb-1 text-sm">Flujo Neto</div>
          <div className={`text-2xl font-bold ${s.netFlow >= 0 ? "text-primary" : "text-orange-600"}`}>
...
export default function CashFlowReport() {
  ...
  const [periodBalance, setPeriodBalance] = useState<{ opening: number; closing: number; currency: string } | null>(null);
  ...
  const load = useCallback(async () => {
    ...
    try {
      const [txRes, sumRes] = await Promise.all([
        cashFlowService.getAll({ ...filters, page, limit: 50, bankAccountId: filters.bankAccountId || undefined }),
  ...
  const exportCsv = async () => {
    ...
    try {
      ...
      const headers = ["Fecha", "Cuenta", "Tipo", "Origen", "Descripción", "Monto", "Moneda", "Saldo Acum."];
      ...
      toast.current?.show({ severity: "error", summary: "Error", detail: "No se pudo exportar" });
    } finally {
      setExporting(false);
    }
  };
  ...
  const amountBody = (row: CashTransaction) => {
    ...
    return (
      <span className={`font-semibold ${color}`}>
        {sign}{row.currency} {Math.abs(Number(row.amount)).toLocaleString("es-VE", { minimumFractionDigits: 2 })}
      </span>
    );
  };
  ...
  const runningBalanceBody = (row: CashTransaction) => {
    ...
    return (
      <span className={`font-medium ${val >= 0 ? "text-900" : "text-red-600"}`}>
        {row.currency} {val.toLocaleString("es-VE", { minimumFractionDigits: 2 })}
      </span>
    );
  };

  const fmt = (v: number) => v.toLocaleString("es-VE", { minimumFractionDigits: 2 });

  const onActionSuccess = async () => {
    await load();
    toast.current?.show({ severity: "success", summary: "Éxito", detail: "Operación registrada correctamente" });
  };
      ...
      <div className="flex gap-2 align-items-end flex-wrap">
        ...
        <div className="flex flex-column gap-1">
          ...
          <Dropdown
            ...
            onChange={(e) => setFilters(f => ({ ...f, bankAccountId: e.value }))}
            className="w-16rem"
          />
        </div>
        <div className="flex flex-column gap-1">
        ...
        <Button label="Filtrar" icon="pi pi-search" onClick={() => { setPage(1); load(); }} />
        <Button
          icon="pi pi-download"
    ...
    <>
      ...
      {periodBalance && filters.bankAccountId && (
        <div className="flex gap-3 mb-4 flex-wrap">
          <div className="surface-100 border-round p-3 flex-1">
            ...
            <div className={`text-xl font-bold ${periodBalance.opening >= 0 ? "text-900" : "text-red-600"}`}>
              {periodBalance.currency} {fmt(periodBalance.opening)}
            </div>
            <div className="text-xs text-500">{filters.from}</div>
          </div>
          <div className="flex align-items-center text-300 text-2xl px-2">→</div>
          <div className="surface-100 border-round p-3 flex-1">
            <div className="text-500 text-sm mb-1">Saldo de Cierre</div>
            <div className={`text-xl font-bold ${periodBalance.closing >= 0 ? "text-primary" : "text-red-600"}`}>
            ...
            {(() => {
              ...
              return (
                <div className={`text-xl font-bold ${diff >= 0 ? "text-green-600" : "text-red-600"}`}>
                  {diff >= 0 ? "+" : ""}{periodBalance.currency} {fmt(diff)}
                </div>
              );
            })()}
          </div>
          ...
          {summary.unified ? (
            <>
              <div className="flex align-items-center gap-2 mb-2">
                <span className="font-semibold text-sm text-600 uppercase">Todo en {summary.unified.currency}</span>
                <span className="text-xs text-500">(conversión usando tasa promedio del período)</span>
              </div>
            ...
            summary.perCurrency.map((s) => (
              <div key={s.currency} className="mb-3">
                <span className="font-semibold text-sm text-600 uppercase block mb-2">{s.currency}</span>
      ...
      >
        <Column
          ...
          header="Fecha"
          body={(r: CashTransaction) => new Date(r.transactionDate).toLocaleDateString("es-VE")}
          style={{ width: "110px" }}
          sortable
        />
        <Column
        ...
        <Column
          ...
          body={(r: CashTransaction) => (
            <Tag
              ...
              severity={r.type === "INCOME" || r.type === "TRANSFER_IN" ? "success" : r.type === "ADJUSTMENT" ? "warning" : "danger"}
            />
          )}
          style={{ width: "140px" }}
        />
        ...
        <Column header="Monto" body={amountBody} style={{ width: "160px", textAlign: "right" }} />
        <Column header="Saldo Acum." body={runningBalanceBody} style={{ width: "160px", textAlign: "right" }} />
      </DataTable>
      </div>
    </>
  );
