...
import type { ServiceOrder, ServiceOrderStatus, UpdateServiceOrderStatusInput } from "@/libs/interfaces/workshop";
...
const BOARD_STATUSES: { id: ServiceOrderStatus; label: string; color: string; icon: string; maxCapacity?: number }[] = [
  { id: "OPEN", label: "Abierta", color: "#3b82f6", icon: "pi pi-folder-open" },
  { id: "DIAGNOSING", label: "Diagnóstico", color: "#8b5cf6", icon: "pi pi-search" },
  { id: "PENDING_APPROVAL", label: "Aprobación", color: "#f59e0b", icon: "pi pi-clock" },
  { id: "APPROVED", label: "Aprobada", color: "#06b6d4", icon: "pi pi-check" },
  { id: "IN_PROGRESS", label: "En proceso", color: "#10b981", icon: "pi pi-wrench", maxCapacity: 8 },
  { id: "PAUSED", label: "Pausada", color: "#f97316", icon: "pi pi-pause" },
  { id: "WAITING_PARTS", label: "Espera piezas", color: "#ef4444", icon: "pi pi-box" },
  { id: "QUALITY_CHECK", label: "Control calidad", color: "#6366f1", icon: "pi pi-shield" },
...
const STATUS_LABELS: Record<string, string> = {
  OPEN: "Abierta", DIAGNOSING: "Diagnóstico", PENDING_APPROVAL: "Aprobación pendiente",
  APPROVED: "Aprobada", IN_PROGRESS: "En proceso", PAUSED: "Pausada",
  WAITING_PARTS: "Esperando piezas", WAITING_AUTH: "Esperando autorización",
  QUALITY_CHECK: "Control de calidad", READY: "Lista", DELIVERED: "Entregada",
  INVOICED: "Facturada", CLOSED: "Cerrada", CANCELLED: "Cancelada", DRAFT: "Borrador",
};
...
function StatusDialog({ order, visible, onHide, onSaved, toast }: StatusDialogProps) {
  ...
  const handleSave = async () => {
    ...
    setSaving(true);
    try {
      await serviceOrderService.updateStatus(order.id, { status: selectedStatus, notes } as UpdateServiceOrderStatusInput);
    ...
    <Dialog
      ...
      footer={
        <div className="flex justify-content-end gap-2">
          <Button label="Cancelar" outlined severity="secondary" onClick={onHide} disabled={saving} />
          <Button label="Guardar" icon="pi pi-check" onClick={handleSave} loading={saving} disabled={!selectedStatus} />
        </div>
      }
    >
      {order && (
        <div className="flex flex-column gap-3">
          ...
          <div className="flex flex-column gap-2">
            {nexts.map((s) => (
              <div
                ...
                className={`p-3 border-round cursor-pointer transition-colors transition-duration-150 ${
                  selectedStatus === s ? "bg-primary-100 border-primary-400" : "surface-100 border-200"
                } border-1`}
            ...
            {nexts.length === 0 && (
              <div className="text-500 text-sm">No hay transiciones disponibles desde este estado.</div>
            )}
          </div>
          <div className="flex flex-column gap-1">
            <label className="text-sm text-600">Notas (opcional)</label>
            <InputText value={notes} onChange={(e) => setNotes(e.target.value)} placeholder="Motivo del cambio..." />
          </div>
        </div>
      )}
    </Dialog>
...
interface ColumnProps {
  cfg: typeof BOARD_STATUSES[number];
  orders: ServiceOrder[];
...
function KanbanColumn({ cfg, orders, onAction }: ColumnProps) {
  ...
  const pct = cfg.maxCapacity ? Math.min(100, Math.round((orders.length / cfg.maxCapacity) * 100)) : null;
      ...
      >
        ...
        {pct !== null && (
          <ProgressBar
            ...
            showValue={false}
            color={pct >= 90 ? "var(--red-400)" : pct >= 70 ? "var(--orange-400)" : undefined}
          />
        )}
      </div>
...
export default function PlanningBoard() {
  ...
  const [statusDialogOrder, setStatusDialogOrder] = useState<ServiceOrder | null>(null);
  ...
  const load = useCallback(async () => {
    ...
    try {
      const params: Record<string, any> = {
        limit: 200,
        status: BOARD_STATUSES.map((s) => s.id).join(","),
      };
  ...
  const ordersByStatus = BOARD_STATUSES.reduce((acc, s) => {
    acc[s.id] = orders.filter((o) => o.status === s.id);
    return acc;
  ...
  const onDragEnd = async ({ active, over }: DragEndEvent) => {
    ...
    try {
      await serviceOrderService.updateStatus(order.id, { status: newStatus } as UpdateServiceOrderStatusInput);
    } catch (err) {
      // Rollback
      setOrders(prev);
      handleFormError(err, toast.current!);
  ...
  const formFooter = (
    <div className="flex justify-content-end gap-2">
      ...
      <Button label={editOrder ? "Guardar" : "Crear OT"} icon="pi pi-check" form="planning-so-form" type="submit" loading={formSubmitting} />
    </div>
  );
      ...
      >
        ...
        <div className="flex flex-wrap justify-content-between align-items-center gap-3 mb-3">
          <div>
            ...
            <span className="text-500 text-sm">{orders.length} órdenes activas</span>
          </div>
          <div className="flex align-items-center gap-2">
            <SelectButton
              value={view}
            ...
            <Button
              label="Nueva OT"
              icon="pi pi-plus"
              onClick={() => { setEditOrder(null); setFormVisible(true); }}
            />
          </div>
        </div>
        ...
        <div className="flex flex-wrap align-items-center gap-2 mb-3">
          ...
          />
          {loading && <i className="pi pi-spin pi-spinner" style={{ fontSize: "1.1rem", color: "var(--primary-color)" }} />}
        </div>
        ...
        <DndContext sensors={sensors} onDragStart={onDragStart} onDragEnd={onDragEnd}>
          ...
          <DragOverlay dropAnimation={{ duration: 150, easing: "ease" }}>
            {activeOrder ? (
              <PlanningKanbanCard order={activeOrder} onAction={() => {}} isOverlay />
            ) : null}
          </DragOverlay>
        </DndContext>
      </motion.div>
