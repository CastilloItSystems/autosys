...
export default function IngressMotiveList() {
  ...
  const editItem = (item: IngressMotive) => { setSelected({ ...item }); setFormDialog(true); };
  const confirmDelete = (item: IngressMotive) => { setSelected(item); setDeleteDialog(true); };
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      toast.current?.show({ severity: "success", summary: "Éxito", detail: "Motivo de ingreso eliminado", life: 3000 });
      await loadItems();
  ...
  const handleToggle = async (item: IngressMotive) => {
    try {
      ...
      toast.current?.show({
        severity: "success",
        summary: "Éxito",
        detail: `Motivo de ingreso ${item.isActive ? "desactivado" : "activado"}`,
        life: 3000,
      });
      await loadItems();
    } catch (error) {
  ...
  const handleSave = () => {
    (async () => {
      toast.current?.show({
        severity: "success",
        summary: "Éxito",
        detail: selected?.id ? "Motivo de ingreso actualizado" : "Motivo de ingreso creado",
        life: 3000,
      });
      await loadItems();
      setFormDialog(false);
  ...
  const header = (
    <div className="flex flex-wrap gap-2 align-items-center justify-content-between">
      ...
      <div className="flex gap-2">
        ...
        <CreateButton label="Nuevo motivo" onClick={openNew} tooltip="Crear motivo de ingreso" />
      </div>
    </div>
  );
          ...
          <div className="mb-2 text-center md:text-left">
            <div className="border-bottom-2 border-primary pb-2">
              <h2 className="text-2xl font-bold text-900 mb-2 flex align-items-center justify-content-center md:justify-content-start">
                ...
                {selected?.id ? "Modificar Motivo de Ingreso" : "Crear Motivo de Ingreso"}
              </h2>
            </div>
          </div>
        }
