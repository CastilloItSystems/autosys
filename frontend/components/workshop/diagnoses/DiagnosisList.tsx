...
export default function DiagnosisList() {
  ...
  const editItem = (item: Diagnosis) => { setSelected({ ...item }); setFormDialog(true); };
  const confirmDelete = (item: Diagnosis) => { setSelected(item); setDeleteDialog(true); };
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      toast.current?.show({ severity: "success", summary: "Éxito", detail: "Diagnóstico eliminado", life: 3000 });
      await loadItems();
  ...
  const referenceBodyTemplate = (rowData: Diagnosis) => (
    <span className="font-bold text-primary">
      {rowData.serviceOrder?.folio ?? rowData.reception?.code ?? rowData.id.slice(0, 8)}
    </span>
  );
  ...
  const severityBodyTemplate = (rowData: Diagnosis) =>
    rowData.severity ? <DiagnosisSeverityBadge severity={rowData.severity} /> : <span className="text-400">—</span>;
  ...
  const header = (
    <div className="flex flex-wrap gap-2 align-items-center justify-content-between">
      ...
      <div className="flex flex-wrap gap-2">
        ...
        <CreateButton label="Nuevo diagnóstico" onClick={openNew} tooltip="Crear diagnóstico" />
      </div>
    </div>
  );
    ...
    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.3 }}>
      ...
      <div className="card">
        <DataTable
          ...
          scrollable
        >
          <Column header="Referencia" body={referenceBodyTemplate} style={{ minWidth: "140px" }} />
          <Column header="Técnico" body={technicianBodyTemplate} style={{ minWidth: "160px" }} />
          <Column header="Severidad" body={severityBodyTemplate} style={{ minWidth: "120px" }} />
          <Column field="status" header="Estado" body={statusBodyTemplate} sortable style={{ minWidth: "130px" }} />
          <Column header="Fecha" body={dateBodyTemplate} style={{ minWidth: "110px" }} />
          <Column
            header="Acciones"
        ...
        model={
          actionItem
            ? [
                ...
                { label: "Ver detalles", icon: "pi pi-eye", command: () => editItem(actionItem) },
              ]
            : []
        }
        popup
