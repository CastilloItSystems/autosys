...
import WorkshopBranchForm from "./WorkshopBranchForm";

export default function WorkshopBranchList() {
  const [items, setItems] = useState<WorkshopBranch[]>([]);
  const [totalRecords, setTotalRecords] = useState(0);
  const [selected, setSelected] = useState<WorkshopBranch | null>(null);
  const [actionItem, setActionItem] = useState<WorkshopBranch | null>(null);
  ...
  const loadItems = async () => {
    try {
      setLoading(true);
      const res = await workshopBranchService.getAll({
        page: page + 1,
        limit: rows,
  ...
  const editItem = (item: WorkshopBranch) => { setSelected({ ...item }); setFormDialog(true); };

  const handleToggle = async (item: WorkshopBranch) => {
    try {
      await workshopBranchService.toggleActive(item.id);
      toast.current?.show({
        severity: "success",
        summary: "Éxito",
        detail: `Sucursal ${item.isActive ? "desactivada" : "activada"}`,
        life: 3000,
      });
      await loadItems();
    } catch (error) {
  ...
  const handleSave = () => {
    (async () => {
      toast.current?.show({
        severity: "success",
        summary: "Éxito",
        detail: selected?.id ? "Sucursal actualizada" : "Sucursal creada",
        life: 3000,
      });
      await loadItems();
      setFormDialog(false);
  ...
  const actionBodyTemplate = (rowData: WorkshopBranch) => (
    <Button
      icon="pi pi-cog"
      rounded
      text
  ...
  const header = (
    <div className="flex flex-wrap gap-2 align-items-center justify-content-between">
      <div className="flex align-items-center gap-2">
        <h4 className="m-0">Sucursales del Taller</h4>
      ...
      <div className="flex gap-2">
        ...
        <CreateButton label="Nueva sucursal" onClick={openNew} tooltip="Crear sucursal" />
      </div>
    </div>
  );
    ...
    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.3 }}>
      ...
      <div className="card">
        ...
        >
          ...
          <Column field="address" header="Dirección" style={{ minWidth: "220px" }} />
          <Column field="phone" header="Teléfono" style={{ minWidth: "130px" }} />
          ...
          <div className="mb-2 text-center md:text-left">
            <div className="border-bottom-2 border-primary pb-2">
              <h2 className="text-2xl font-bold text-900 mb-2 flex align-items-center justify-content-center md:justify-content-start">
                <i className="pi pi-building mr-3 text-primary text-3xl" />
                {selected?.id ? "Modificar Sucursal" : "Crear Sucursal"}
              </h2>
            </div>
          </div>
        }
