...
export default function WorkshopBayList() {
  ...
  const handleToggle = async (item: WorkshopBay) => {
    try {
      await workshopBayService.toggleActive(item.id);
      toast.current?.show({
        severity: "success",
        summary: "Éxito",
  ...
  const header = (
    <div className="flex flex-wrap gap-2 align-items-center justify-content-between">
      <div className="flex align-items-center gap-2">
        <h4 className="m-0">Bahías de Taller</h4>
    ...
    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.3 }}>
      ...
      <div className="card">
        <DataTable
          ...
          loading={loading}
          header={header}
          emptyMessage="No se encontraron bahías"
          sortMode="multiple"
          scrollable
        >
          ...
          <div className="mb-2 text-center md:text-left">
            <div className="border-bottom-2 border-primary pb-2">
              <h2 className="text-2xl font-bold text-900 mb-2 flex align-items-center justify-content-center md:justify-content-start">
                <i className="pi pi-th-large mr-3 text-primary text-3xl" />
                {selected?.id ? "Modificar Bahía" : "Crear Bahía"}
              </h2>
            </div>
          </div>
        }
