...
export default function WorkshopOperationList() {
  ...
  const editItem = (item: WorkshopOperation) => { setSelected({ ...item }); setFormDialog(true); };
  const confirmDelete = (item: WorkshopOperation) => { setSelected(item); setDeleteDialog(true); };
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      toast.current?.show({ severity: "success", summary: "Éxito", detail: "Operación eliminada", life: 3000 });
      await loadItems();
  ...
  const priceBodyTemplate = (rowData: WorkshopOperation) =>
    new Intl.NumberFormat("es-MX", { style: "currency", currency: "MXN" }).format(rowData.listPrice);
  ...
  const header = (
    <div className="flex flex-wrap gap-2 align-items-center justify-content-between">
      ...
      <div className="flex gap-2">
        ...
        <CreateButton label="Nueva operación" onClick={openNew} tooltip="Crear operación de taller" />
      </div>
    </div>
  );
    ...
    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.3 }}>
      ...
      <div className="card">
        ...
        >
          ...
          <Column field="serviceType" header="Tipo de servicio" body={serviceTypeBodyTemplate} style={{ minWidth: "160px" }} />
          <Column field="standardMinutes" header="Min. estándar" body={minutesBodyTemplate} style={{ minWidth: "120px" }} />
          <Column field="listPrice" header="Precio lista" body={priceBodyTemplate} sortable style={{ minWidth: "130px" }} />
      ...
      <Dialog
        visible={formDialog}
        style={{ width: "560px" }}
        breakpoints={{ "900px": "70vw", "600px": "90vw" }}
        maximizable
        header={
