...
import { serviceOrderService, serviceTypeService, workshopBayService } from "@/app/api/workshop";
import {
  createServiceOrderSchema,
  updateServiceOrderSchema,
  type CreateServiceOrderForm,
} from "@/libs/zods/workshop/serviceOrderZod";
import type { ServiceOrder, ServiceType, WorkshopBay } from "@/libs/interfaces/workshop";
...
export default function ServiceOrderForm({
  ...
  const {
    ...
    resolver: zodResolver(order ? updateServiceOrderSchema : createServiceOrderSchema),
    mode: "onBlur",
    defaultValues: {
      customerId: "",
      priority: "NORMAL",
  ...
  return (
    <form id={formId ?? "service-order-form"} onSubmit={handleSubmit(onSubmit)} className="p-fluid">
      <div className="grid">
        ...
        <div className="col-12 md:col-6">
          <label htmlFor="diagnosisNotes" className="block text-900 font-medium mb-2">
            Diagnóstico
          </label>
          <Controller
            name="diagnosisNotes"
        ...
        <div className="col-12">
          <Divider align="left">
            <span className="text-700 font-semibold text-sm">Ítems de la orden</span>
          </Divider>
        </div>
        ...
        <div className="col-12">
          <div className="flex flex-column gap-2">
            {fields.map((field, index) => (
              <div key={field.id} className="p-3 surface-100 border-round grid m-0 gap-2 align-items-end">
                  ...
                  <Controller
                    ...
                    render={({ field: f }) => (
                      <Dropdown
                        ...
                        placeholder="Tipo"
                        className={errors.items?.[index]?.type ? "p-invalid" : ""}
                      />
                    )}
                  />
                </div>
                <div className="col-12 md:col-4 p-0">
                  <label className="block text-700 text-sm mb-1">Descripción</label>
                  <Controller
                    ...
                    render={({ field: f }) => (
                      <InputText
                        ...
                        className={errors.items?.[index]?.description ? "p-invalid" : ""}
                      />
                    )}
                  />
                  {errors.items?.[index]?.description && (
                    <small className="p-error">{errors.items[index]?.description?.message}</small>
                  )}
                </div>
                  ...
                  <Controller
                    ...
                    render={({ field: f }) => (
                      <InputNumber
                        ...
                        minFractionDigits={0}
                        maxFractionDigits={2}
                        className={errors.items?.[index]?.quantity ? "p-invalid" : ""}
                      />
                    )}
                  />
                </div>
                <div className="col-6 md:col-2 p-0">
                  <label className="block text-700 text-sm mb-1">Precio unit.</label>
                  <Controller
                    ...
                    render={({ field: f }) => (
                      <InputNumber
                        ...
                        minFractionDigits={2}
                        min={0}
                        className={errors.items?.[index]?.unitPrice ? "p-invalid" : ""}
                      />
                    )}
                  />
                </div>
                <div className="col-6 md:col-1 p-0">
                  <label className="block text-700 text-sm mb-1">Dto. %</label>
                  <Controller
            ...
            <Button
              ...
              onClick={() => append({ type: "LABOR", description: "", quantity: 1, unitPrice: 0, discountPct: 0 })}
            />
          </div>
        </div>
