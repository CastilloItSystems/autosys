...
const InvoiceListContent = () => {
  ...
  const fiscalBodyTemplate = (rowData: Invoice) => (
    <span className="text-sm font-mono">{rowData.fiscalNumber || "—"}</span>
  );
              ...
              <div className="font-bold text-900">
                ...
                {data.payment?.method && (
                  <span className="text-xs text-500 ml-2">
                    (
                    {PAYMENT_METHOD_CONFIG[data.payment.method as PaymentMethod]
    ...
    <>
      ...
      >
        ...
        >
          ...
          <Column field="invoiceNumber" header="Nro. Factura" sortable />
          <Column
      ...
      >
        <div className="flex flex-column gap-3 p-2">
          <div className="flex align-items-center gap-3 p-2 surface-100 border-round">
            ...
            <div>
              <span>
                ¿Anular la factura <b>{selectedInvoice?.invoiceNumber}</b>?
              </span>
              <div className="text-xs text-500 mt-1">
                La factura quedará registrada como anulada (SENIAT no permite
                eliminar).
              </div>
            </div>
          </div>
