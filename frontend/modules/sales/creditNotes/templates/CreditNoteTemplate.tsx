...
const CreditNoteTemplate = ({ data, company }: { data: CreditNote; company?: PdfCompanyInfo }) => {
  const cfg = CREDIT_NOTE_STATUS_CONFIG[data.status];
  const badgeColor = statusBadgeColors[data.status] || statusBadgeColors.ACTIVE;

  return (
    <Document title={`Nota de Crédito - ${data.creditNoteNumber}`}>
      <Page size="A4" style={styles.page}>
        ...
        {/* Referencia a factura y motivo */}
        <View style={styles.section}>
          <Text style={styles.sectionTitle}>Referencia</Text>
          <View style={styles.grid2Col}>
            <View style={styles.col}>
              <View style={styles.row}>
                <Text style={styles.label}>Factura vinculada:</Text>
                <Text style={styles.value}>{data.invoice?.invoiceNumber || "—"}</Text>
              </View>
              {data.invoice?.fiscalNumber && (
                <View style={styles.row}>
                  <Text style={styles.label}>Nro. Control:</Text>
                  <Text style={styles.value}>{data.invoice.fiscalNumber}</Text>
                </View>
              )}
            </View>
            <View style={styles.col}>
        ...
        <View style={styles.totalsBox}>
          ...
          {data.igtfApplies && (
            <View style={styles.totalLine}>
              <Text style={styles.totalLabel}>IGTF ({data.igtfRate}%):</Text>
