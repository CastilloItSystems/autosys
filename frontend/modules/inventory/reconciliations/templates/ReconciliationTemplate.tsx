...
const styles = StyleSheet.create({
  ...
  // Timeline
  timeline: { flexDirection: "row", justifyContent: "space-between", marginVertical: 8 },
  ...
  timelineDotInactive: { backgroundColor: "#e2e8f0" },
  timelineLabel: { fontSize: 7, color: "#64748b", textAlign: "center" },
  timelineValue: { fontSize: 7, color: "#1e293b", textAlign: "center", fontFamily: "Roboto-Bold" },
  timelineConnector: {
    position: "absolute",
    top: 5,
    left: "50%",
...
const ReconciliationTemplate = ({ data }: { data: Reconciliation }) => {
  ...
  const sourceCfg = data.source
    ? RECONCILIATION_SOURCE_CONFIG[data.source]
    : null;

  const timelineSteps: TimelineStep[] = [
    { label: "Iniciado", date: data.startedAt, active: !!data.startedAt },
    { label: "Completado", date: data.completedAt, active: !!data.completedAt },
    { label: "Aprobado", date: data.approvedAt, active: !!data.approvedAt },
    { label: "Aplicado", date: data.appliedAt, active: !!data.appliedAt },
  ];
    ...
    <Document title={`Reconciliación - ${data.reconciliationNumber}`}>
      <Page size="A4" style={styles.page}>
            ...
            {timelineSteps.map((step, idx) => (
              <View key={idx} style={styles.timelineItem}>
                <View
                  style={[
                    styles.timelineDot,
                    step.active ? {} : styles.timelineDotInactive,
                  ]}
                />
                <Text style={styles.timelineLabel}>{step.label}</Text>
                <Text style={styles.timelineValue}>
                  {step.active ? formatDate(step.date) : "Pendiente"}
                </Text>
              </View>
            ))}
          </View>
          ...
          <View style={styles.grid2Col}>
            <View style={styles.col}>
              ...
              <View style={styles.row}>
                ...
                <Text style={styles.value}>
                  {sourceCfg ? sourceCfg.label : data.source || "—"}
                </Text>
              </View>
              <View style={styles.row}>
        ...
        <View style={styles.section}>
          ...
          <View style={styles.table}>
            ...
            {items.map((item, idx) => {
              ...
              const diffColor =
                diff < 0 ? "#dc2626" : diff > 0 ? "#16a34a" : "#334155";
              return (
                <View
                  key={item.id}
                  style={[
        ...
        <View style={styles.totalsBox}>
          ...
          <View style={styles.totalLine}>
            <Text style={styles.totalLabel}>Ítems con diferencia:</Text>
            <Text style={styles.totalValue}>{String(itemsWithDiff.length)}</Text>
          </View>
        </View>
...
export default ReconciliationTemplate;
