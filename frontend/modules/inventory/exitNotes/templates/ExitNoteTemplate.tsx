...
} from "../interfaces/exitNote.interface";
...
const ExitNoteTemplate = ({ data }: { data: ExitNote }) => {
  const cfg = EXIT_NOTE_STATUS_CONFIG[data.status];
  const badgeColor = statusBadgeColors[data.status] || statusBadgeColors.PENDING;
  const typeCfg = EXIT_NOTE_TYPE_CONFIG[data.type];

  return (
    <Document title={`Nota de Salida - ${data.exitNoteNumber}`}>
      <Page size="A4" style={styles.page}>
        {/* Header */}
        <View style={styles.header} fixed>
          <View style={styles.headerLeft}>
            <View>
              <Text style={styles.headerTitle}>Nota de Salida</Text>
              <Text style={styles.headerSubtitle}>AutoSys</Text>
            </View>
          </View>
          <View style={styles.headerRight}>
            <Text style={styles.headerNumber}>{data.exitNoteNumber}</Text>
            ...
            <Text style={styles.headerDate}>Tipo: {typeCfg?.label || data.type}</Text>
          </View>
        </View>
        ...
        <View style={styles.section}>
          ...
          <View style={styles.grid2Col}>
            <View style={styles.col}>
              ...
              {data.recipientId && (
                <View style={styles.row}>
                  <Text style={styles.label}>ID Destinatario:</Text>
                  <Text style={styles.value}>{data.recipientId}</Text>
                </View>
              )}
            </View>
            <View style={styles.col}>
              ...
              {data.expectedReturnDate && (
                <View style={styles.row}>
                  <Text style={styles.label}>Devolución esperada:</Text>
                  <Text style={styles.value}>{formatDate(data.expectedReturnDate)}</Text>
                </View>
              )}
            </View>
          </View>
        ...
        {data.items && data.items.length > 0 && (
          <View style={styles.section}>
            <Text style={styles.sectionTitle}>Artículos Enviados</Text>
            <View style={styles.table}>
              <View style={styles.tableHeader}>
                ...
                <Text style={[styles.tableHeaderCell, { width: "24%" }]}>Ubicación Origen</Text>
                <Text style={[styles.tableHeaderCell, { width: "24%" }]}>Notas</Text>
              </View>
        ...
        <View style={styles.signatureBlock}>
          <Text style={styles.signatureLine}>Preparó</Text>
          <Text style={styles.signatureLine}>Entregó</Text>
        ...
        <View style={styles.footer} fixed>
          ...
          <Text style={styles.footerText}>{data.exitNoteNumber}</Text>
          <Text
...
export default ExitNoteTemplate;
