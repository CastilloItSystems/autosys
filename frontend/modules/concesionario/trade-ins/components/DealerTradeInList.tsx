...
} from "../utils/dealerTradeIn.utils";
...
export default function DealerTradeInList() {
  ...
  const handleSave = async () => {
    toast.current?.show({
      ...
      summary: "Éxito",
      detail: selected?.id
        ? "Retoma actualizada correctamente"
  ...
  const header = (
    <div className="flex flex-wrap gap-2 align-items-center justify-content-between">
      ...
      <div className="flex flex-wrap gap-2">
        <Dropdown
          value={statusFilter}
          options={TRADE_IN_STATUS_FILTER_OPTIONS}
          onChange={(e) => {
      ...
      >
        ...
        <Column
          ...
          body={(row: DealerTradeIn) => {
            const meta = TRADE_IN_STATUS_META[row.status] || {
              label: row.status,
