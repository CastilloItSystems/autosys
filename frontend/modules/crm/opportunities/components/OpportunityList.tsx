...
import {
  OPPORTUNITY_CHANNEL_FILTER_OPTIONS as CHANNEL_OPTIONS,
  OPPORTUNITY_STATUS_FILTER_OPTIONS as STATUS_OPTIONS,
  OPPORTUNITY_STAGE_FLOW as STAGE_FLOW,
  getOpportunityFlow as getFlow,
...
export default function OpportunityList() {
  ...
  const moveStage = async (row: Opportunity, stageCode: string) => {
    try {
      ...
      toast.current?.show({
        severity: "error",
        summary: e?.response?.data?.message ?? "No se pudo mover la etapa",
      });
    }
  };
  ...
  const closeOpportunity = async (row: Opportunity, result: "WON" | "LOST") => {
    if (result === "LOST") {
      confirmDialog({
        ...
        accept: async () => {
          try {
            await opportunityService.close(row.id, {
              result: "LOST",
            ...
            toast.current?.show({
              severity: "error",
              summary: "No se pudo cerrar la oportunidad",
            });
          }
        },
      });
      ...
      {view === "kanban" ? (
              ...
              >
                ...
                <div className="flex flex-column gap-2">
                  {(groupedByStage[stage] || []).map((row) => (
                    <div
                      key={row.id}
                      className="p-2 border-1 border-round border-200"
                    >
                      ...
                      <div className="flex justify-content-between align-items-center mb-2">
                        <Tag
                          value={
                            OPPORTUNITY_CHANNEL_CONFIG[
                              row.channel as keyof typeof OPPORTUNITY_CHANNEL_CONFIG
                            ]?.label ?? row.channel
                          }
                        />
                      ...
                      <Dropdown
                        ...
                        options={flowForCurrentChannel.map((s) => ({
                          label: s,
                          value: s,
                        }))}
                        optionLabel="label"
                      ...
                      <div className="flex gap-1 mt-2">
                        <Button
                          ...
                          severity="success"
                          tooltip="Ganada"
                          onClick={() => closeOpportunity(row, "WON")}
                        />
                        <Button
                          icon="pi pi-times"
                          text
                          severity="danger"
                          tooltip="Perdida"
                          onClick={() => closeOpportunity(row, "LOST")}
                        />
                        <Button
                          icon="pi pi-eye"
                          text
                  ...
                  {(groupedByStage[stage] || []).length === 0 && (
                    <small className="text-400">Sin oportunidades</small>
                  )}
                </div>
              </div>
            ))}
        ...
        >
          ...
          <Column
            ...
            body={(row: Opportunity) => (
              <Tag
                value={
                  OPPORTUNITY_STATUS_CONFIG[
                    row.status as keyof typeof OPPORTUNITY_STATUS_CONFIG
                  ]?.label ?? row.status
                }
              />
            )}
          ...
          <Column
            ...
            header="Próx. actividad"
            body={(row: Opportunity) =>
              new Date(row.nextActivityAt).toLocaleString("es-VE")
            }
          />
          <Column
            header="Acciones"
        ...
        {!detailData ? (
          ...
          <div className="grid">
            <div className="col-12 md:col-6">
              <strong>Título:</strong> {detailData.title}
            </div>
            <div className="col-12 md:col-6">
              <strong>Estado:</strong> {detailData.status}
            </div>
            <div className="col-12 md:col-6">
              <strong>Etapa:</strong> {detailData.stageCode}
            </div>
            <div className="col-12 md:col-6">
              <strong>Owner:</strong> {detailData.ownerId}
            </div>
            <div className="col-12">
              <strong>Descripción:</strong> {detailData.description || "—"}
            </div>
            <div className="col-12">
