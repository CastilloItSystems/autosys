...
import type {
  WorkshopWarranty,
  WarrantyStatus,
} from "@/modules/workshop/warranties/interfaces/warranty.interface";
import {
  WarrantyStatusBadge,
...
export default function WarrantyStatusDialog({
  visible,
  warranty,
  onHide,
  onSaved,
  ...
  const handleSave = async () => {
    ...
    setSaving(true);
    try {
      await warrantyService.updateStatus(warranty.id, newStatus);
      await onSaved();
    } catch (error) {
  ...
  const availableOptions = warranty
    ? WARRANTY_STATUS_OPTIONS.filter((opt) =>
        VALID_TRANSITIONS[warranty.status].includes(
          opt.value as WarrantyStatus,
        ),
      )
    : [];
      ...
      {warranty && (
        <div className="flex flex-column gap-4 pt-2">
          <div className="flex align-items-center gap-3">
            <span className="text-600 font-medium" style={{ width: "5rem" }}>
              Actual:
            </span>
          ...
          <div className="flex flex-column gap-2">
            ...
            {availableOptions.length > 0 ? (
              ...
              <div className="p-3 border-round surface-100 text-sm text-600 flex align-items-center gap-2">
                ...
                <span>
                  Esta garantía está cerrada y no puede cambiar de estado.
                </span>
              </div>
            )}
          </div>
