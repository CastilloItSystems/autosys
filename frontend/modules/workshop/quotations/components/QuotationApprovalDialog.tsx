...
import type {
  ...
  ApprovalChannel,
  ApprovalType,
} from "@/modules/workshop/quotations/interfaces/quotation.interface";
...
export default function QuotationApprovalDialog({
  visible,
  quotation,
  onHide,
  onSaved,
  ...
  const footer = (
    <div className="flex justify-content-end gap-2">
      ...
      <Button
        label={
          type === "REJECTION" ? "Registrar rechazo" : "Registrar aprobación"
        }
        icon="pi pi-check"
  ...
  return (
    ...
    >
      <div className="p-fluid flex flex-column gap-3">
        ...
        {type === "PARTIAL" && quotationItems.length > 0 && (
          <div>
            <div className="flex align-items-center justify-content-between mb-2">
              <label className="block font-semibold text-sm m-0">
              ...
              <div className="flex gap-2">
                <Button
                  ...
                  size="small"
                  onClick={() =>
                    setApprovedItemIds(quotationItems.map((it) => it.id))
                  }
                />
                <Button
                  type="button"
            ...
            <div
              className="border-1 border-200 border-round p-2"
            ...
            >
              {quotationItems.map((item) => {
                ...
                return (
                  <div
                    key={item.id}
                    className="flex align-items-start gap-2 py-2 border-bottom-1 border-100"
                  >
                    <Checkbox
                    ...
                    >
                      ...
                      <span className="text-600">
                        {" "}
                        — Cant: {item.quantity} · Total: {item.total.toFixed(2)}
                      </span>
                    </label>
                  </div>
                );
            ...
            {approvedItemIds.length === 0 && (
              <small className="p-error block mt-2">
                Selecciona al menos un ítem para aprobación parcial.
              </small>
            )}
          </div>
        )}
