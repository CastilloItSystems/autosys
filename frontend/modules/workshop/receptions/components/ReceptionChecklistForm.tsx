...
import {
  useChecklistResponsesData,
  useChecklistTemplateData,
} from "@/modules/workshop/checklists/hooks";
...
export default function ReceptionChecklistForm({
  ...
  const {
    ...
    error: templateError,
  } = useChecklistTemplateData(templateId);
  const {
    responses: savedResponses,
    loading: savedResponsesLoading,
  } = useChecklistResponsesData(receptionId);

  // Cargar respuestas guardadas cuando se monta el componente
  useEffect(() => {
    if (savedResponsesLoading || savedResponses.length === 0) return;

    const mappedResponses = new Map<string, ChecklistResponse>();
    savedResponses.forEach((resp) => {
      mappedResponses.set(resp.checklistItemId, {
        checklistItemId: resp.checklistItemId,
        boolValue: resp.boolValue,
        textValue: resp.textValue,
        numValue: resp.numValue != null ? Number(resp.numValue) : undefined,
        selectionValue: resp.selectionValue,
        observation: resp.observation,
      });
    });
    setResponses(mappedResponses);
  }, [savedResponses, savedResponsesLoading]);
  ...
  const handleResponseChange = (
    ...
    switch (item.responseType) {
      case "BOOLEAN":
        newResponse.boolValue =
          value === true ? true : value === false ? false : null;
        break;
      case "TEXT":
        newResponse.textValue = typeof value === "string" ? value : null;
        break;
      case "NUMBER":
        newResponse.numValue = value != null ? Number(value) : null;
        break;
      case "SELECTION":
        newResponse.selectionValue = typeof value === "string" ? value : null;
        break;
    }
  ...
  if (templateLoading || savedResponsesLoading) {
    return (
