...
} from "../interfaces/workshopShift.interface";
import type {
...
const workshopShiftService = {
  async getAll(
    filters?: WorkshopShiftFilters,
  ): Promise<WorkshopPagedResponse<WorkshopShift>> {
  ...
  async create(
    data: CreateWorkshopShiftInput,
  ): Promise<WorkshopResponse<WorkshopShift>> {
