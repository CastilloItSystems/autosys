...
} from "@/modules/workshop/shared/components/DynamicChecklist";
import type { ChecklistTemplate } from '@/modules/workshop/checklists/interfaces/checklist.interface';;
  ...
  const onSubmit = async (data: FormData) => {
    ...
    try {
      ...
      if (checklistResponses.length > 0) {
        payload.checklistItems = checklistItems.map((item) => {
          const response = checklistResponses.find(
            (r) => r.checklistItemId === item.id,
          );
          const notes = response?.observation?.trim();

          return {
            key: item.code || item.id,
            label: item.name,
            passed: response?.boolValue ?? false,
