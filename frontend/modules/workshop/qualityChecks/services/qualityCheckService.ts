...
} from '@/modules/workshop/qualityChecks/interfaces/qualityCheck.interface';
...
const qualityCheckService = {
  async getAll(params?: {
    ...
    page?: number;
    limit?: number;
  }): Promise<WorkshopPagedResponse<QualityCheck>> {
  ...
  async getByOrder(
    serviceOrderId: string,
  ): Promise<WorkshopResponse<QualityCheck>> {
    const res = await apiClient.get(`${BASE}/by-order/${serviceOrderId}`);
    return res.data;
  },
