...
import type {
  ...
  GaritaEventType,
  GaritaEventStatus,
} from "@/modules/workshop/garita/interfaces/garita.interface";
...
const STATUS_OPTIONS = (
  Object.keys(GARITA_STATUS_LABELS) as GaritaEventStatus[]
).map((s) => ({
  label: GARITA_STATUS_LABELS[s],
  value: s,
}));

const TYPE_OPTIONS = (Object.keys(GARITA_TYPE_LABELS) as GaritaEventType[]).map(
  (t) => ({
    ...
    <div className="flex align-items-center gap-2">
      ...
      {row.hasIrregularity && (
        <i
          ...
          title={row.irregularityNotes ?? "Irregularidad"}
        />
      )}
    </div>
  );
