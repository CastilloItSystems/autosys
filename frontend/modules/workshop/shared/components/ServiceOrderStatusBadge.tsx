...
import type { ServiceOrderStatus } from "@/modules/workshop/serviceOrders/interfaces/serviceOrder.interface";
import type { ServiceOrderPriority } from "@/modules/workshop/serviceOrders/interfaces/serviceOrder.interface";
...
export function ServiceOrderPriorityBadge({
  priority,
  rounded = true,
}: ServiceOrderPriorityBadgeProps) {
  return (
    <Tag
...
export const SO_STATUS_OPTIONS = (
  Object.keys(SO_STATUS_LABELS) as ServiceOrderStatus[]
).map((v) => ({
  label: SO_STATUS_LABELS[v],
  value: v,
}));

export const SO_PRIORITY_OPTIONS = (
  Object.keys(SO_PRIORITY_LABELS) as ServiceOrderPriority[]
).map((v) => ({
  label: SO_PRIORITY_LABELS[v],
  value: v,
}));

export default ServiceOrderStatusBadge;
