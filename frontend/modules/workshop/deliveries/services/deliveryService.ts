// app/api/workshop/deliveryService.ts
import apiClient from "@/app/api/apiClient";
import type { VehicleDelivery, DeliveryFilters, CreateDeliveryInput } from '@/modules/workshop/deliveries/interfaces/delivery.interface';
import type { WorkshopResponse } from '@/modules/workshop/shared/interfaces/shared.interface';
const BASE = "/workshop/deliveries";

const deliveryService = {
  async getAll(
    filters?: DeliveryFilters,
  ): Promise<WorkshopResponse<VehicleDelivery[]>> {
    const res = await apiClient.get(BASE, { params: filters });
    return res.data;
  },

  async getById(id: string): Promise<WorkshopResponse<VehicleDelivery>> {
    const res = await apiClient.get(`${BASE}/${id}`);
    return res.data;
  },

  async create(
    data: CreateDeliveryInput,
  ): Promise<WorkshopResponse<VehicleDelivery>> {
    const res = await apiClient.post(BASE, data);
    return res.data;
  },

  // FASE 1.2: Update delivery
  async update(
    id: string,
    data: Partial<CreateDeliveryInput>,
  ): Promise<WorkshopResponse<VehicleDelivery>> {
    const res = await apiClient.patch(`${BASE}/${id}`, data);
    return res.data;
  },

  // FASE 1.3: Delete delivery
  async delete(id: string): Promise<WorkshopResponse<void>> {
    const res = await apiClient.delete(`${BASE}/${id}`);
    return res.data;
  },

  async getByOrder(
    orderId: string,
  ): Promise<WorkshopResponse<VehicleDelivery>> {
    const res = await apiClient.get(`${BASE}/order/${orderId}`);
    return res.data;
  },
};

export default deliveryService;
