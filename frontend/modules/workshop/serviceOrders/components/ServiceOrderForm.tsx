...
} from "@/modules/workshop/serviceOrders/schemas/serviceOrderZod";
...
import { SO_PRIORITY_OPTIONS } from "@/modules/workshop/shared/components/ServiceOrderStatusBadge";
...
export default function ServiceOrderForm({
  ...
  useEffect(() => {
    if (isLoading) return;
    if (order) {
      const hydratedCatalogMap = (order.items ?? []).reduce((acc, i: any) => {
      ...
      <div className="grid">
        ...
        <div className="col-12">
          <WorkshopFinancialSummary
            ...
            currency={watchCurrency}
            exchangeRate={
              (watch("exchangeRate" as any) as number | null) ?? null
            }
          />
        </div>
      </div>
