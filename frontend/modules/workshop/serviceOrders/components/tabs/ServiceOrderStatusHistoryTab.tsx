...
import type { ServiceOrderStatusHistoryEntry } from "@/modules/workshop/serviceOrders/interfaces/serviceOrder.interface";
...
export default function ServiceOrderStatusHistoryTab({
  serviceOrderId,
}: Props) {
      ...
      >
        <i
          className={`pi pi-arrow-right ${
            isTerminal ? "text-green-600" : "text-blue-600"
          }`}
        />
      </span>
    );
  ...
  const content = (entry: ServiceOrderStatusHistoryEntry) => {
    ...
    return (
      <div className="pb-3">
        ...
        {entry.comment && (
          <p className="m-0 mt-2 text-sm text-700">{entry.comment}</p>
        )}
      </div>
    );
  };
