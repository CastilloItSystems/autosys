...
import type { DiagnosisFinding } from "@/modules/workshop/diagnoses/interfaces/diagnosis.interface";
...
export default function DiagnosisFindingsTab({
  diagnosisId,
  findings,
  onRefresh,
  toast,
  ...
  const severityTemplate = (row: DiagnosisFinding) => (
    <Tag
      value={SEVERITY_LABELS[row.severity]}
      severity={SEVERITY_TAG[row.severity]}
    />
  );
  ...
  const authTemplate = (row: DiagnosisFinding) =>
    row.requiresClientAuth ? (
      <i
        className="pi pi-check-circle text-orange-500"
        ...
        <span className="text-sm text-500">
          {findings.length === 0
            ...
            : `${findings.length} hallazgo${
                findings.length !== 1 ? "s" : ""
              } registrado${findings.length !== 1 ? "s" : ""}.`}
        </span>
        <Button
        ...
        <div className="flex flex-column gap-3 p-fluid pt-2">
          ...
          <div className="flex align-items-center gap-2">
            <Checkbox
              ...
              onChange={(e) =>
                setForm((f) => ({ ...f, requiresClientAuth: !!e.checked }))
              }
            />
            <label htmlFor="reqAuth" className="text-sm">
