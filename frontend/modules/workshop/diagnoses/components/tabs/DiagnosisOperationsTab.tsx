...
import type { DiagnosisSuggestedOp } from "@/modules/workshop/diagnoses/interfaces/diagnosis.interface";
...
export default function DiagnosisOperationsTab({
  diagnosisId,
  operations,
  onRefresh,
  toast,
  ...
  const minsTemplate = (row: DiagnosisSuggestedOp) =>
    row.estimatedMins ? (
      <span>{row.estimatedMins} min</span>
    ) : (
    ...
    <div className="pt-2">
      <div className="flex justify-content-between align-items-center mb-3">
        <span className="text-sm text-500">
          {operations.length === 0
            ? "No hay operaciones sugeridas."
            : `${operations.length} operación${
                operations.length !== 1 ? "es" : ""
              } sugerida${operations.length !== 1 ? "s" : ""}.`}
        </span>
        <Button
          ...
          <div className="grid formgrid">
            <div className="col-12 md:col-6">
              ...
              <InputNumber
                ...
                onValueChange={(e) =>
                  setForm((f) => ({ ...f, estimatedMins: e.value ?? 0 }))
                }
                min={0}
                className="w-full"
              />
            ...
            <div className="col-12 md:col-6">
              ...
              <InputNumber
                ...
                onValueChange={(e) =>
                  setForm((f) => ({ ...f, estimatedPrice: e.value ?? 0 }))
                }
                min={0}
                mode="currency"
                currency="USD"
