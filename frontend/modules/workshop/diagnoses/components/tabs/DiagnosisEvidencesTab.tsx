...
export default function DiagnosisEvidencesTab({
  diagnosisId,
  evidences,
  onRefresh,
  toast,
  ...
  const isValid = useFileUpload
    ? !!selectedFile || !!form.url.trim()
    : !!form.url.trim();
            ...
            >
              <div className="flex align-items-center gap-3 min-w-0">
                {ev.type === "photo" ? (
                  ...
                  <i
                    className={`${TYPE_ICON[ev.type] ?? "pi pi-link"} text-xl ${
                      TYPE_COLOR[ev.type] ?? "text-500"
                    }`}
                  />
                )}
        ...
        <div className="flex flex-column gap-3 p-fluid pt-2">
          ...
          {!selectedFile && (
            <div>
              <label className="text-sm font-semibold block mb-1">
                O pega una URL{" "}
                {selectedFile ? "" : <span className="text-red-500">*</span>}
              </label>
              <InputText
                value={form.url}
                onChange={(e) =>
