...
const RifInput: React.FC<RifInputProps> = ({
  ...
  const availableTypes = React.useMemo(() => {
    if (customerType === "COMPANY") {
      return RIF_TYPES.filter((t) => ["J", "G"].includes(t.value));
    }
    if (customerType === "INDIVIDUAL") {
      return RIF_TYPES.filter((t) => ["V", "E", "P"].includes(t.value));
    }
    return RIF_TYPES;
  }, [customerType]);
  ...
  useEffect(() => {
    if (availableTypes.length > 0 && !availableTypes.some((t) => t.value === rifType)) {
      const newType = availableTypes[0].value;
      setRifType(newType);
      if (onChange) {
         onChange(`${newType}-${rifNumber}`);
      }
    }
  }, [availableTypes, rifType, rifNumber, onChange]);
